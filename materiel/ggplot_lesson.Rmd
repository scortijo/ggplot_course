...
La fonction `read_csv()` imprime un message indiquant quel type de données sont contenues dans les differentes colonnes du fichier. 

Dans notre cas, certaines colonnes contiennes des données de type "character" (du texte) et d'autres des données numériques ("double" en présence de décimale, "integer" en absence de décimale).
...
Une autre option est d'utiliser la fonction  ``View()`` pour accéder à une table intéractive où il est possible de trier et filtrer les données sans modifier l'objet:
...
Il faut d'abord mettre le graphique dans un objet, puis de l'utiliser dans la fonction `ggplotly()`.


```{r go interactive}
# Garder le graphique dans l'objet p1
